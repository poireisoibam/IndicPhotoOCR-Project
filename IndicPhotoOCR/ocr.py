...
class OCR:
    ...
    def ocr(self, image_path):
        ...
        output = {}
        for idx, (key, data) in enumerate(recognized_texts.items()):
            try:
                output[f"polygon_{idx}"] = {
                    "coordinates": data["bbox"],               # [x1, y1, x2, y2]
                    "text": data["txt"],                       # recognized text
                    "script_language": data["lang"]            # correct language
                }
            except Exception as e:
                if self.verbose:
                    print(f"⚠️ Error formatting polygon {idx}: {e}")



        return output  # correct structured format
